...
class MyHandler(http.server.CGIHTTPRequestHandler):
    ...
    def do_POST(self):
        self.log_t()
        form = self.parse_POST()  # what is 'form'???
        self.log_t()

        if "id" in form:
            id_str = form["id"][0]
            id_str = re.sub(r'\W+', '', id_str.decode())
            print('LJW str: ' + id_str)
        else:
            self.ret_result(False)
            return

        print('LJW path: ' + self.path)
    ...
    def post_process(self, form, id_str):
        ...
        if "line" in form:
            bin1 = form["line"][0]
            fout1 = open("./images/line/" + id_str + ".png", 'wb')  # write in binary
            fout1.write(bin1)
            fout1.close()
        else:
            self.ret_result(False)
    ...
    def paint_process(self, form, id_str):
        ...
        if "blur" in form:
            ...
            try:
                ...
                blur = 0
        print('LJW blur: ' + str(blur))

        if "step" in form:
            print('LJW step: ' + form["step"][0].decode())
        self.log_t()
